...
import NavbarForUser from '../../../components/UserComponents/NavbarForUser/NavbarForUser'

const UserDashboard = () => {
  const [userData,setUserData] = useState({})
 
  const headers = {
    "Authorization":`Bearer ${Cookies.get("userJwtToken")}`,
  ...
  const getUserData = async () =>{
    try {
      ...
      const response = await axios.get(`http://localhost:8000/getuserdetails`,{headers})
